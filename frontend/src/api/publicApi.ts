...
export async function updateGuestLanguage(guestSessionId: string, languageId: number): Promise<void> {
  await http.patch(`${endpoints.guestSessions}/${guestSessionId}/language`, { languageId });
}
...
export async function getNarrationsByPlace(placeId: number): Promise<NarrationContentDTO[]> {
  const response = await http.get(`${endpoints.narrationContents}/place/${placeId}`);
  return unwrap<NarrationContentDTO[]>(response);
}

export async function getNarrationsByDish(dishId: number): Promise<NarrationContentDTO[]> {
  const response = await http.get(`${endpoints.narrationContents}/dish/${dishId}`);
  return unwrap<NarrationContentDTO[]>(response);
}

export async function getTranslation(narrationId: number, languageId: number): Promise<NarrationTranslationDTO> {
  const response = await http.get(`${endpoints.narrationTranslations}/narration/${narrationId}/language/${languageId}`);
  return unwrap<NarrationTranslationDTO>(response);
}

export async function getPlayableAudio(narrationId: number, languageId: number): Promise<AudioFileDTO | null> {
  try {
    const response = await http.get(`${endpoints.audioFiles}/playable`, { params: { narrationId, languageId } });
...
export async function resolvePlaceNarration(placeId: number, languageId: number): Promise<NarrationResolveResultDTO> {
  const narrations = await getNarrationsByPlace(placeId);
  const narration = narrations.find((x) => x.isActive) ?? narrations[0];
  if (!narration) throw new Error('Không tìm thấy nội dung thuyết minh cho địa điểm này.');
...
export async function resolveDishNarration(dishId: number, languageId: number): Promise<NarrationResolveResultDTO> {
  const narrations = await getNarrationsByDish(dishId);
  const narration = narrations.find((x) => x.isActive) ?? narrations[0];
  if (!narration) throw new Error('Không tìm thấy nội dung thuyết minh cho món ăn này.');
...
export async function resolveQr(qrCodeValue: string, languageId: number, guestSessionId: string): Promise<NarrationResolveResultDTO> {
  const response = await http.post(endpoints.qrResolve, { qrCodeValue, languageId, guestSessionId });
...
export async function checkGeofence(guestSessionId: string, latitude: number, longitude: number, languageId: number): Promise<GeofenceCheckResultDTO> {
  const response = await http.post(endpoints.geofenceCheck, { guestSessionId, latitude, longitude, languageId });
  return unwrap<GeofenceCheckResultDTO>(response);
}

export async function submitListeningHistory(payload: ListeningHistoryDTO): Promise<void> {
