export type UiLanguage = 'vi' | 'en' | 'ja' | 'ko' | 'zh';

const keyTranslations: Record<UiLanguage, Record<string, string>> = {
  vi: {
    'language.vi': 'Tiếng Việt',
    'language.en': 'English',
    'language.ja': '日本語',
'language.ko': '한국어',
'language.zh': '中文',
    'language.switch': 'Ngôn ngữ',
    'language.current': 'Ngôn ngữ hiện tại',

    'public.settings.title': 'Cài đặt',
    'public.settings.uiLanguage': 'Ngôn ngữ giao diện',
    'public.settings.uiLanguageDescription': 'Chỉ thay đổi chữ và nút trên giao diện, không thay đổi ngôn ngữ thuyết minh.',
    'public.settings.noSession': 'Chưa có',
    'public.settings.noLanguage': 'Chưa chọn',
    'public.settings.changeLanguage': 'Đổi ngôn ngữ',
    'public.settings.tracking': 'Theo dõi vị trí',
    'public.settings.trackingDescription': 'Có thể bật/tắt trực tiếp ở màn hình bản đồ.',
        'public.settings.guestSession': 'Phiên khách',
    'public.settings.narrationLanguage': 'Ngôn ngữ',
'public.settings.noNarrationLanguage': 'Chưa chọn',
'public.settings.narrationLanguageDescription': 'Dùng để tải văn bản và audio thuyết minh đúng ngôn ngữ bạn chọn.',
'public.settings.changeNarrationLanguage': 'Đổi ngôn ngữ',
    'public.settings.enableTracking': 'Bật theo dõi',
    'public.settings.disableTracking': 'Tắt theo dõi',

    'public.languageSelection.title': 'Chọn ngôn ngữ',
'public.languageSelection.description': 'Ngôn ngữ này sẽ được dùng cho giao diện, bản dịch và giọng đọc thuyết minh.',
    'public.languageSelection.loading': 'Đang tải ngôn ngữ...',
    'public.languageSelection.empty': 'Chưa có ngôn ngữ khả dụng.',
    'public.languageSelection.choose': 'Chọn',

    'public.map.title': 'Bản đồ phố ẩm thực',
    'public.map.defaultStatus': 'Mặc định hiển thị khu vực phố ẩm thực Vĩnh Khánh.',
    'public.map.placesLoaded': 'Địa điểm đã tải',
    'public.map.placesVisible': 'Sạp đang hiển thị',
    'public.map.searchPlaceholder': 'Tìm theo tên sạp...',
    'public.map.clearSearch': 'Xóa bộ lọc',
    'public.map.noSearchResults': 'Không tìm thấy sạp phù hợp.',
    'public.map.nearestPoi': 'POI gần nhất',
    'public.map.distance': 'Khoảng cách',
    'public.map.distanceFromYou': 'Cách vị trí của bạn',
    'public.map.locationDistanceHint': 'Nhấn nút vị trí để tính khoảng cách đến POI gần nhất.',
    'public.map.enableTracking': 'Bật theo dõi vị trí',
    'public.map.disableTracking': 'Tắt theo dõi vị trí',
    'public.map.noAddress': 'Chưa có địa chỉ',
    'public.map.noDescription': 'Chưa có mô tả cho địa điểm này.',
    'public.map.openingHours': 'Giờ mở cửa',
    'public.map.geofenceRadius': 'Bán kính geofence',
    'public.map.dishesTitle': 'Món ăn / bảng giá',
    'public.map.noDishes': 'Chưa có món ăn cho quán này.',
    'public.map.dish': 'Món',
    'public.map.recommended': 'Gợi ý',
    'public.map.price': 'Giá',
    'public.map.notUpdated': 'Chưa cập nhật',
    'public.map.sendFeedback': 'Gửi feedback cho quán',
    'public.map.contentLanguageUnavailable': 'Chưa có nội dung đã duyệt cho ngôn ngữ đang chọn.',
    'public.map.audioNotReady': 'Audio cho ngôn ngữ đang chọn chưa sẵn sàng.',
    'public.map.narrationUnavailable': 'Địa điểm hoặc món ăn này chưa có thuyết minh công khai.',
    'public.map.loadingNarration': 'Đang tải nội dung thuyết minh...',


    'public.map.locationRequested': 'Đã xin quyền vị trí. Nếu trình duyệt cho phép, bản đồ sẽ focus theo vị trí của bạn.',
    'public.map.trackingStarted': 'Đang theo dõi vị trí và gửi geofence check theo thời gian thực.',
    'public.map.trackingStopped': 'Đã tắt theo dõi vị trí.',

    'public.player.back': 'Quay lại',
    'public.player.backToMap': 'Quay lại bản đồ',
    'public.player.play': 'Nghe',
    'public.player.pause': 'Tạm dừng',
    'public.player.stop': 'Dừng',
    'public.player.sendFeedback': 'Gửi feedback',
    'public.player.noNarration': 'Chưa có nội dung thuyết minh.',

    'public.feedback.title': 'Gửi đánh giá',
    'public.feedback.placeholder': 'Cảm nhận của bạn...',
    'public.feedback.sending': 'Đang gửi...',
    'public.feedback.submit': 'Gửi feedback',

    'public.nav.map': 'Bản đồ',
'public.nav.settings': 'Cài đặt',

'public.player.source.geofence': 'Tự động theo vị trí',
'public.player.source.place': 'Địa điểm',
'public.player.source.dish': 'Món ăn',
'public.player.source.narration': 'Thuyết minh',
'public.player.source.manual': 'Thủ công',
'public.player.audioFile': 'Audio file',
'public.player.audioUnavailable': 'Audio chưa sẵn sàng',
'public.player.audioUnavailableMessage': 'Audio cho ngôn ngữ này chưa được tạo hoặc chưa được công khai.',
'public.player.audioLoadError': 'Không thể tải file audio. Vui lòng thử lại sau.',
'public.player.seek': 'Tua audio',

'Chọn dữ liệu': 'Chọn dữ liệu',
'Không chọn Place': 'Không chọn địa điểm',
'Không chọn Dish': 'Không chọn món ăn',
'Không chọn Narration': 'Không chọn thuyết minh',

'Content Type': 'Loại nội dung',
'Place Type': 'Loại địa điểm',
'Target Type': 'Loại đối tượng',
'Trigger Mode': 'Chế độ kích hoạt',
'Translation Source': 'Nguồn dịch',

'Category': 'Danh mục',
'Place': 'Địa điểm',
'Dish': 'Món ăn',
'Narration': 'Thuyết minh',
'Translation': 'Bản dịch',
'Language': 'Ngôn ngữ',
'Active': 'Hoạt động',
'Actions': 'Thao tác',
'Value': 'Giá trị',
'Id': 'Mã',

'Edit': 'Sửa',
'Hide': 'Ẩn',
'Create': 'Tạo mới',
'Yes': 'Có',
'No': 'Không',

'Unable to load dropdown data. Please check lookup backend endpoints.': 'Không tải được dữ liệu dropdown. Kiểm tra backend endpoint lookup.',
'Không tải được dữ liệu dropdown. Kiểm tra backend endpoint lookup.': 'Không tải được dữ liệu dropdown. Kiểm tra backend endpoint lookup.',
'Restore': 'Mở lại',
'Save Changes': 'Lưu thay đổi',
'public.feedback.success': 'Cảm ơn bạn đã gửi đánh giá.',
'public.feedback.error': 'Không gửi được đánh giá. Vui lòng thử lại.',
  },

  en: {
    'language.vi': 'Vietnamese',
    'language.en': 'English',
    'language.ja': 'Japanese',
    'language.ko': 'Korean',
    'language.zh': 'Chinese',
    'language.switch': 'Language',
    'language.current': 'Current language',

    'public.settings.title': 'Settings',
    'public.settings.noSession': 'Not available',
    'public.settings.noLanguage': 'Not selected',
    'public.settings.changeLanguage': 'Change language',
    'public.settings.tracking': 'Location tracking',
    'public.settings.trackingDescription': 'You can enable or disable it directly on the map screen.',
        'public.settings.guestSession': 'Guest Session',
    'public.settings.uiLanguage': 'Interface language',
    'public.settings.uiLanguageDescription': 'Changes interface text and buttons only. It does not change the narration language.',
    'public.settings.narrationLanguage': 'Language',
'public.settings.noNarrationLanguage': 'Not selected',
'public.settings.narrationLanguageDescription': 'Used to load narration text and audio in the selected language.',
'public.settings.changeNarrationLanguage': 'Change language',
    'public.settings.enableTracking': 'Enable tracking',
    'public.settings.disableTracking': 'Disable tracking',

    'public.languageSelection.title': 'Choose language',
'public.languageSelection.description': 'This language will be used for the interface, translations, and narration voice.',
    'public.languageSelection.loading': 'Loading languages...',
    'public.languageSelection.empty': 'No available languages.',
    'public.languageSelection.choose': 'Choose',

    'public.map.title': 'Food street map',
    'public.map.defaultStatus': 'Showing the default Vinh Khanh food street area.',
    'public.map.placesLoaded': 'Places loaded',
    'public.map.placesVisible': 'Places shown',
    'public.map.searchPlaceholder': 'Search by stall name...',
    'public.map.clearSearch': 'Clear filter',
    'public.map.noSearchResults': 'No matching stall was found.',
    'public.map.nearestPoi': 'Nearest POI',
    'public.map.distance': 'Distance',
    'public.map.distanceFromYou': 'Distance from you',
    'public.map.locationDistanceHint': 'Tap the location button to calculate the nearest POI.',
    'public.map.enableTracking': 'Enable location tracking',
    'public.map.disableTracking': 'Disable location tracking',
    'public.map.noAddress': 'No address available',
    'public.map.noDescription': 'No description is available for this place.',
    'public.map.openingHours': 'Opening hours',
    'public.map.geofenceRadius': 'Geofence radius',
    'public.map.dishesTitle': 'Dishes / Price list',
    'public.map.noDishes': 'No dishes are available for this place.',
    'public.map.dish': 'Dish',
    'public.map.recommended': 'Recommended',
    'public.map.price': 'Price',
    'public.map.notUpdated': 'Not updated',
    'public.map.sendFeedback': 'Send feedback for this place',
    'public.map.contentLanguageUnavailable': 'Approved content is not available in the selected language.',
    'public.map.audioNotReady': 'Audio is not ready for the selected language.',
    'public.map.narrationUnavailable': 'No published narration is available for this place or dish.',
    'public.map.loadingNarration': 'Loading narration...',


    'public.map.locationRequested': 'Location permission has been requested. If allowed, the map will focus on your position.',
    'public.map.trackingStarted': 'Location tracking is active and geofence checks are being sent in real time.',
    'public.map.trackingStopped': 'Location tracking has been disabled.',

    'public.player.back': 'Back',
    'public.player.backToMap': 'Back to map',
    'public.player.play': 'Play',
    'public.player.pause': 'Pause',
    'public.player.stop': 'Stop',
    'public.player.sendFeedback': 'Send feedback',
    'public.player.noNarration': 'No narration content is available.',

    'public.feedback.title': 'Send feedback',
    'public.feedback.placeholder': 'Share your thoughts...',
    'public.feedback.sending': 'Sending...',
    'public.feedback.submit': 'Submit feedback',
    'public.nav.map': 'Map',
'public.nav.settings': 'Settings',

'public.player.source.geofence': 'Location-based',
'public.player.source.place': 'Place',
'public.player.source.dish': 'Dish',
'public.player.source.narration': 'Narration',
'public.player.source.manual': 'Manual',
'public.player.audioFile': 'Audio file',
'public.player.audioUnavailable': 'Audio unavailable',
'public.player.audioUnavailableMessage': 'Audio for this language has not been generated or published yet.',
'public.player.audioLoadError': 'The audio file could not be loaded. Please try again later.',
'public.player.seek': 'Seek audio',

'Chọn dữ liệu': 'Select data',
'Không chọn Place': 'No Place',
'Không chọn Dish': 'No Dish',
'Không chọn Narration': 'No Narration',

'Content Type': 'Content Type',
'Place Type': 'Place Type',
'Target Type': 'Target Type',
'Trigger Mode': 'Trigger Mode',
'Translation Source': 'Translation Source',

'Category': 'Category',
'Place': 'Place',
'Dish': 'Dish',
'Narration': 'Narration',
'Translation': 'Translation',
'Language': 'Language',
'Active': 'Active',
'Actions': 'Actions',
'Value': 'Value',
'Id': 'Id',

'Edit': 'Edit',
'Hide': 'Hide',
'Create': 'Create',
'Yes': 'Yes',
'No': 'No',

'Unable to load dropdown data. Please check lookup backend endpoints.': 'Unable to load dropdown data. Please check lookup backend endpoints.',
'Không tải được dữ liệu dropdown. Kiểm tra backend endpoint lookup.': 'Unable to load dropdown data. Please check lookup backend endpoints.',
'Restore': 'Restore',
'Save Changes': 'Save Changes',
'public.feedback.success': 'Thank you for your feedback.',
'public.feedback.error': 'Unable to send feedback. Please try again.',
  },
    ja: {
    'language.vi': 'ベトナム語',
    'language.en': '英語',
    'language.ja': '日本語',
    'language.ko': '韓国語',
    'language.zh': '中国語',
    'language.switch': '言語',
    'language.current': '現在の言語',

    'public.settings.title': '設定',
    'public.settings.uiLanguage': '表示言語',
    'public.settings.uiLanguageDescription': '画面の文字とボタンのみを変更し、ナレーション言語は変更しません。',
    'public.settings.noSession': 'なし',
    'public.settings.noLanguage': '未選択',
    'public.settings.changeLanguage': '言語を変更',
    'public.settings.tracking': '位置情報の追跡',
    'public.settings.trackingDescription': '地図画面で直接オン／オフを切り替えられます。',
    'public.settings.guestSession': 'ゲストセッション',
    'public.settings.narrationLanguage': '言語',
    'public.settings.noNarrationLanguage': '未選択',
    'public.settings.narrationLanguageDescription': '選択した言語のナレーション文章と音声を読み込みます。',
    'public.settings.changeNarrationLanguage': '言語を変更',
    'public.settings.enableTracking': '追跡を有効にする',
    'public.settings.disableTracking': '追跡を無効にする',

    'public.languageSelection.title': '言語を選択',
    'public.languageSelection.description': 'この言語は、画面表示、翻訳、音声ガイドに使用されます。',
    'public.languageSelection.loading': '言語を読み込み中...',
    'public.languageSelection.empty': '利用可能な言語がありません。',
    'public.languageSelection.choose': '選択',

    'public.map.title': 'グルメ通りマップ',
    'public.map.defaultStatus': 'Vinh Khanh グルメ通りエリアを表示しています。',
    'public.map.placesLoaded': '読み込まれた場所',
    'public.map.placesVisible': '表示中の店舗',
    'public.map.searchPlaceholder': '店舗名で検索...',
    'public.map.clearSearch': '検索をクリア',
    'public.map.noSearchResults': '一致する店舗が見つかりません。',
    'public.map.nearestPoi': '最寄りの POI',
    'public.map.distance': '距離',
    'public.map.distanceFromYou': '現在地からの距離',
    'public.map.locationDistanceHint': '位置ボタンを押すと最寄りの POI までの距離を計算します。',
    'public.map.enableTracking': '位置情報の追跡を有効にする',
    'public.map.disableTracking': '位置情報の追跡を無効にする',
    'public.map.noAddress': '住所がありません',
    'public.map.noDescription': 'この場所の説明はまだありません。',
    'public.map.openingHours': '営業時間',
    'public.map.geofenceRadius': 'ジオフェンス半径',
    'public.map.dishesTitle': '料理 / 価格表',
    'public.map.noDishes': 'この場所の料理情報はまだありません。',
    'public.map.dish': '料理',
    'public.map.recommended': 'おすすめ',
    'public.map.price': '価格',
    'public.map.notUpdated': '未更新',
    'public.map.sendFeedback': 'この場所にフィードバックを送信',
    'public.map.contentLanguageUnavailable': '選択した言語の承認済みコンテンツはまだありません。',
    'public.map.audioNotReady': '選択した言語の音声はまだ準備できていません。',
    'public.map.narrationUnavailable': 'この場所または料理の公開ナレーションはありません。',
    'public.map.loadingNarration': 'ナレーションを読み込んでいます...',


    'public.map.locationRequested': '位置情報の許可をリクエストしました。許可されると地図が現在地に移動します。',
    'public.map.trackingStarted': '位置情報の追跡が有効になり、ジオフェンス確認を送信しています。',
    'public.map.trackingStopped': '位置情報の追跡を停止しました。',

    'public.player.back': '戻る',
    'public.player.backToMap': '地図に戻る',
    'public.player.play': '再生',
    'public.player.pause': '一時停止',
    'public.player.stop': '停止',
    'public.player.sendFeedback': 'フィードバックを送信',
    'public.player.noNarration': '音声ガイドの内容がありません。',

    'public.feedback.title': '評価を送信',
    'public.feedback.placeholder': '感想を入力してください...',
    'public.feedback.sending': '送信中...',
    'public.feedback.submit': 'フィードバックを送信',

    'public.nav.map': '地図',
'public.nav.settings': '設定',

'public.player.source.geofence': '位置情報による自動再生',
'public.player.source.place': '場所',
'public.player.source.dish': '料理',
'public.player.source.narration': '音声ガイド',
'public.player.source.manual': '手動',
'public.player.audioFile': '音声ファイル',
'public.player.audioUnavailable': '音声はまだ利用できません',
'public.player.audioUnavailableMessage': 'この言語の音声はまだ生成または公開されていません。',
'public.player.audioLoadError': '音声ファイルを読み込めませんでした。後でもう一度お試しください。',
'public.player.seek': '音声をシーク',
'public.feedback.success': '評価を送信していただきありがとうございます。',
'public.feedback.error': '評価を送信できませんでした。もう一度お試しください。',
  },

  ko: {
    'language.vi': '베트남어',
    'language.en': '영어',
    'language.ja': '일본어',
    'language.ko': '한국어',
    'language.zh': '중국어',
    'language.switch': '언어',
    'language.current': '현재 언어',

    'public.settings.title': '설정',
    'public.settings.uiLanguage': '인터페이스 언어',
    'public.settings.uiLanguageDescription': '화면의 글자와 버튼만 변경하며 내레이션 언어는 변경하지 않습니다.',
    'public.settings.noSession': '없음',
    'public.settings.noLanguage': '선택되지 않음',
    'public.settings.changeLanguage': '언어 변경',
    'public.settings.tracking': '위치 추적',
    'public.settings.trackingDescription': '지도 화면에서 직접 켜거나 끌 수 있습니다.',
    'public.settings.guestSession': '게스트 세션',
    'public.settings.narrationLanguage': '언어',
    'public.settings.noNarrationLanguage': '선택되지 않음',
    'public.settings.narrationLanguageDescription': '선택한 언어의 내레이션 텍스트와 오디오를 불러옵니다.',
    'public.settings.changeNarrationLanguage': '언어 변경',
    'public.settings.enableTracking': '추적 켜기',
    'public.settings.disableTracking': '추적 끄기',

    'public.languageSelection.title': '언어 선택',
    'public.languageSelection.description': '이 언어는 화면 표시, 번역, 음성 안내에 사용됩니다.',
    'public.languageSelection.loading': '언어를 불러오는 중...',
    'public.languageSelection.empty': '사용 가능한 언어가 없습니다.',
    'public.languageSelection.choose': '선택',

    'public.map.title': '음식 거리 지도',
    'public.map.defaultStatus': 'Vinh Khanh 음식 거리 기본 구역을 표시하고 있습니다.',
    'public.map.placesLoaded': '불러온 장소',
    'public.map.placesVisible': '표시 중인 가게',
    'public.map.searchPlaceholder': '가게 이름으로 검색...',
    'public.map.clearSearch': '필터 지우기',
    'public.map.noSearchResults': '일치하는 가게가 없습니다.',
    'public.map.nearestPoi': '가장 가까운 POI',
    'public.map.distance': '거리',
    'public.map.distanceFromYou': '현재 위치에서의 거리',
    'public.map.locationDistanceHint': '위치 버튼을 눌러 가장 가까운 POI까지의 거리를 계산하세요.',
    'public.map.enableTracking': '위치 추적 켜기',
    'public.map.disableTracking': '위치 추적 끄기',
    'public.map.noAddress': '주소 없음',
    'public.map.noDescription': '이 장소에 대한 설명이 아직 없습니다.',
    'public.map.openingHours': '영업시간',
    'public.map.geofenceRadius': '지오펜스 반경',
    'public.map.dishesTitle': '음식 / 가격표',
    'public.map.noDishes': '이 장소의 음식 정보가 아직 없습니다.',
    'public.map.dish': '음식',
    'public.map.recommended': '추천',
    'public.map.price': '가격',
    'public.map.notUpdated': '업데이트되지 않음',
    'public.map.sendFeedback': '이 장소에 피드백 보내기',
    'public.map.contentLanguageUnavailable': '선택한 언어의 승인된 콘텐츠가 없습니다.',
    'public.map.audioNotReady': '선택한 언어의 오디오가 아직 준비되지 않았습니다.',
    'public.map.narrationUnavailable': '이 장소나 음식에 공개된 내레이션이 없습니다.',
    'public.map.loadingNarration': '내레이션을 불러오는 중...',


    'public.map.locationRequested': '위치 권한을 요청했습니다. 허용되면 지도가 현재 위치로 이동합니다.',
    'public.map.trackingStarted': '위치 추적이 켜졌으며 지오펜스 확인을 전송하고 있습니다.',
    'public.map.trackingStopped': '위치 추적이 꺼졌습니다.',

    'public.player.back': '뒤로',
    'public.player.backToMap': '지도로 돌아가기',
    'public.player.play': '듣기',
    'public.player.pause': '일시정지',
    'public.player.stop': '정지',
    'public.player.sendFeedback': '피드백 보내기',
    'public.player.noNarration': '해설 내용이 없습니다.',

    'public.feedback.title': '평가 보내기',
    'public.feedback.placeholder': '의견을 입력하세요...',
    'public.feedback.sending': '전송 중...',
    'public.feedback.submit': '피드백 보내기',

    'public.nav.map': '지도',
'public.nav.settings': '설정',

'public.player.source.geofence': '위치 기반 자동 재생',
'public.player.source.place': '장소',
'public.player.source.dish': '음식',
'public.player.source.narration': '해설',
'public.player.source.manual': '수동',
'public.player.audioFile': '오디오 파일',
'public.player.audioUnavailable': '오디오를 아직 사용할 수 없습니다',
'public.player.audioUnavailableMessage': '이 언어의 오디오는 아직 생성되거나 공개되지 않았습니다.',
'public.player.audioLoadError': '오디오 파일을 불러올 수 없습니다. 나중에 다시 시도해 주세요.',
'public.player.seek': '오디오 탐색',
'public.feedback.success': '평가를 보내주셔서 감사합니다.',
'public.feedback.error': '평가를 보낼 수 없습니다. 다시 시도해 주세요.',
  },

  zh: {
    'language.vi': '越南语',
    'language.en': '英语',
    'language.ja': '日语',
    'language.ko': '韩语',
    'language.zh': '中文',
    'language.switch': '语言',
    'language.current': '当前语言',

    'public.settings.title': '设置',
    'public.settings.uiLanguage': '界面语言',
    'public.settings.uiLanguageDescription': '只更改界面文字和按钮，不更改讲解语言。',
    'public.settings.noSession': '暂无',
    'public.settings.noLanguage': '未选择',
    'public.settings.changeLanguage': '更改语言',
    'public.settings.tracking': '位置跟踪',
    'public.settings.trackingDescription': '可在地图页面直接开启或关闭。',
    'public.settings.guestSession': '访客会话',
    'public.settings.narrationLanguage': '语言',
    'public.settings.noNarrationLanguage': '未选择',
    'public.settings.narrationLanguageDescription': '用于加载所选语言的讲解文字和音频。',
    'public.settings.changeNarrationLanguage': '更改语言',
    'public.settings.enableTracking': '开启跟踪',
    'public.settings.disableTracking': '关闭跟踪',

    'public.languageSelection.title': '选择语言',
    'public.languageSelection.description': '该语言将用于界面、翻译和语音讲解。',
    'public.languageSelection.loading': '正在加载语言...',
    'public.languageSelection.empty': '暂无可用语言。',
    'public.languageSelection.choose': '选择',

    'public.map.title': '美食街地图',
    'public.map.defaultStatus': '正在显示 Vinh Khanh 美食街默认区域。',
    'public.map.placesLoaded': '已加载地点',
    'public.map.placesVisible': '当前显示的摊位',
    'public.map.searchPlaceholder': '按摊位名称搜索...',
    'public.map.clearSearch': '清除筛选',
    'public.map.noSearchResults': '未找到匹配的摊位。',
    'public.map.nearestPoi': '最近的 POI',
    'public.map.distance': '距离',
    'public.map.distanceFromYou': '距您的位置',
    'public.map.locationDistanceHint': '点击定位按钮以计算到最近 POI 的距离。',
    'public.map.enableTracking': '开启位置跟踪',
    'public.map.disableTracking': '关闭位置跟踪',
    'public.map.noAddress': '暂无地址',
    'public.map.noDescription': '该地点暂无说明。',
    'public.map.openingHours': '营业时间',
    'public.map.geofenceRadius': '地理围栏半径',
    'public.map.dishesTitle': '菜品 / 价格表',
    'public.map.noDishes': '该地点暂无菜品信息。',
    'public.map.dish': '菜品',
    'public.map.recommended': '推荐',
    'public.map.price': '价格',
    'public.map.notUpdated': '未更新',
    'public.map.sendFeedback': '给该地点发送反馈',


    'public.map.locationRequested': '已请求位置权限。如浏览器允许，地图将定位到您的位置。',
    'public.map.trackingStarted': '位置跟踪已开启，并正在发送地理围栏检查。',
    'public.map.trackingStopped': '位置跟踪已关闭。',

    'public.player.back': '返回',
    'public.player.backToMap': '返回地图',
    'public.player.play': '播放',
    'public.player.pause': '暂停',
    'public.player.stop': '停止',
    'public.player.sendFeedback': '发送反馈',
    'public.player.noNarration': '暂无讲解内容。',

    'public.feedback.title': '发送评价',
    'public.feedback.placeholder': '请输入您的感想...',
    'public.feedback.sending': '正在发送...',
    'public.feedback.submit': '发送反馈',

    'public.nav.map': '地图',
'public.nav.settings': '设置',

'public.player.source.geofence': '基于位置自动播放',
'public.player.source.place': '地点',
'public.player.source.dish': '菜品',
'public.player.source.narration': '讲解',
'public.player.source.manual': '手动',
'public.player.audioFile': '音频文件',
'public.player.audioUnavailable': '音频尚不可用',
'public.player.audioUnavailableMessage': '此语言的音频尚未生成或发布。',
'public.player.audioLoadError': '无法加载音频文件，请稍后重试。',
'public.player.seek': '拖动音频进度',
'public.feedback.success': '感谢您提交评价。',
'public.feedback.error': '无法发送评价，请重试。',
  }
};

const viText: Record<string, string> = {
  Admin: 'Quản trị',
  Dashboard: 'Bảng điều khiển',
  Lookup: 'Danh mục',
  Languages: 'Ngôn ngữ',
  'Places / POI': 'Địa điểm / POI',
  Dishes: 'Món ăn',
  Narrations: 'Thuyết minh',
  Translations: 'Bản dịch',
  Audio: 'Âm thanh',
  'QR Codes': 'Mã QR',
  Feedbacks: 'Feedback',
  Feedback: 'Feedback',
  Listening: 'Lượt nghe',
  Geofence: 'Geofence',

  Language: 'Ngôn ngữ',
  Vietnamese: 'Tiếng Việt',
  English: 'English',

  'Admin Dashboard': 'Bảng điều khiển quản trị',
  'Language Management': 'Quản lý ngôn ngữ',
  'The narration languages are fixed for the system. Admin cannot create new languages on this screen.': 'Danh sách ngôn ngữ thuyết minh được cố định cho hệ thống. Admin không tạo mới ngôn ngữ tại màn hình này.',
'Supported system languages': 'Ngôn ngữ hệ thống hỗ trợ',
'These languages are used for narration translations and guest language selection.': 'Các ngôn ngữ này sẽ được dùng cho bản dịch thuyết minh và lựa chọn ngôn ngữ nghe của khách.',
'Display Name': 'Tên hiển thị',
'System Label': 'Nhãn hệ thống',
'TTS lang': 'TTS lang',
'DB Status': 'Trạng thái DB',
'Active in DB': 'Đang hoạt động',
'Exists in DB but disabled': 'Có trong DB nhưng đang tắt',
'Missing in DB': 'Chưa có trong DB',
'Languages outside fixed configuration': 'Ngôn ngữ không thuộc cấu hình cố định',
'These rows should be disabled by seed_supported_languages_fixed.sql. They should not be used for the public app.': 'Các dòng này nên được tắt bằng file seed_supported_languages_fixed.sql. Không nên dùng cho app public.',
  'Lookup Management': 'Quản lý danh mục',
  'Places / POI Management': 'Quản lý địa điểm / POI',
  'Dish Categories': 'Danh mục món ăn',
  'Place Dishes': 'Món ăn theo địa điểm',
  'Narration Management': 'Quản lý thuyết minh',
  'Translation Management': 'Quản lý bản dịch',
  'Audio Management': 'Quản lý âm thanh',
  'QR Code Management': 'Quản lý mã QR',
  'Feedback Management': 'Quản lý feedback',
  'Geofence Events': 'Sự kiện geofence',
  'Listening Histories': 'Lịch sử nghe',

  'Place Types': 'Loại địa điểm',
  'Content Types': 'Loại nội dung',
  'Target Types': 'Loại đối tượng',
  'Translation Sources': 'Nguồn dịch',
  'Trigger Modes': 'Chế độ kích hoạt',
  'Geofence Event Types': 'Loại sự kiện geofence',
  'Geofence Event Statuses': 'Trạng thái sự kiện geofence',

  'Thêm dữ liệu': 'Thêm dữ liệu',
  'Cập nhật dữ liệu': 'Cập nhật dữ liệu',
  'Tạo mới': 'Tạo mới',
  'Lưu thay đổi': 'Lưu thay đổi',
  'Hủy': 'Hủy',
  'Sửa': 'Sửa',
  'Ẩn': 'Ẩn',
  'Mở': 'Mở',
  'Thao tác': 'Thao tác',

  'Add data': 'Thêm dữ liệu',
  'Update data': 'Cập nhật dữ liệu',
  Create: 'Tạo mới',
  'Save changes': 'Lưu thay đổi',
  Cancel: 'Hủy',
  Edit: 'Sửa',
  Hide: 'Ẩn',
  Restore: 'Mở',
  Actions: 'Thao tác',
  Approve: 'Duyệt',
  Reject: 'Từ chối',
  'Update status': 'Cập nhật trạng thái',

  Yes: 'Có',
  No: 'Không',
  On: 'Bật',
  Off: 'Tắt',

  Id: 'Id',
  ID: 'Id',
  Code: 'Mã',
  Name: 'Tên',
  Default: 'Mặc định',
  Active: 'Hoạt động',
  Description: 'Mô tả',
  Address: 'Địa chỉ',
  Latitude: 'Vĩ độ',
  Longitude: 'Kinh độ',
  Lat: 'Vĩ độ',
  Lng: 'Kinh độ',
  Price: 'Giá',
  Category: 'Danh mục',
  Recommended: 'Gợi ý',
  Status: 'Trạng thái',
  Rating: 'Đánh giá',
  Comment: 'Bình luận',
  Note: 'Ghi chú',
  Type: 'Loại',
  Source: 'Nguồn',
  Target: 'Đối tượng',
  Guest: 'Khách',
  Distance: 'Khoảng cách',
  Detected: 'Thời điểm phát hiện',
  Duration: 'Thời lượng',
  Time: 'Thời gian',
  URL: 'URL',
  Format: 'Định dạng',
  Value: 'Giá trị',
  Approved: 'Đã duyệt',

  'Language Code': 'Mã ngôn ngữ',
  'Language Name': 'Tên ngôn ngữ',
  'Language Id': 'Id ngôn ngữ',

  'Place Id': 'Id địa điểm',
  'Place Name': 'Tên địa điểm',
  Place: 'Địa điểm',
  'Place Type Id': 'Id loại địa điểm',
  'Opening Hours': 'Giờ mở cửa',
  'Image URL': 'URL hình ảnh',
  'Is POI': 'Là POI',
  'Geofence Enabled': 'Bật geofence',
  'Radius Meters': 'Bán kính mét',
  Priority: 'Độ ưu tiên',
  'Trigger Mode Id': 'Id chế độ kích hoạt',
  'Debounce Seconds': 'Thời gian debounce',
  'Cooldown Seconds': 'Thời gian cooldown',

  'Dish Id': 'Id món ăn',
  'Dish Name': 'Tên món ăn',
  Dish: 'Món ăn',
  'Category Id': 'Id danh mục',
  'Category Name': 'Tên danh mục',
  'Average Price': 'Giá trung bình',
  'Signature Dish': 'Món đặc trưng',
  Signature: 'Đặc trưng',

  'Narration Id': 'Id thuyết minh',
  Narration: 'Thuyết minh',
  'Original Text': 'Nội dung gốc',
  'Content Type Id': 'Id loại nội dung',
  'Created By Admin Id': 'Id admin tạo',

  'Translation Id': 'Id bản dịch',
  Translation: 'Bản dịch',
  'Translated Title': 'Tiêu đề đã dịch',
  'Translated Text': 'Nội dung đã dịch',
  'Translation Source Id': 'Id nguồn dịch',
  'Reviewed By': 'Người duyệt',
  Reviewed: 'Đã duyệt',

  'Audio URL': 'URL âm thanh',
  'AudioUrl': 'URL âm thanh',
  'Voice Name': 'Tên giọng đọc',
  'Voice Gender': 'Giới tính giọng đọc',
  'Duration Seconds': 'Thời lượng giây',
  'File Format': 'Định dạng file',
  'Generated By': 'Tạo bởi',

  'QR Code Value': 'Giá trị mã QR',
  'QR Image URL': 'URL hình QR',
  'Target Type Id': 'Id loại đối tượng',
  'Target Type': 'Loại đối tượng',

  'Tự động lấy từ địa chỉ': 'Tự động lấy từ địa chỉ',
  'Lấy tọa độ từ địa chỉ': 'Lấy tọa độ từ địa chỉ',
  'Vui lòng nhập địa chỉ trước.': 'Vui lòng nhập địa chỉ trước.',
  'Auto-fill from address or enter manually': 'Tự động lấy từ địa chỉ hoặc nhập tay',
  'Không tìm thấy tọa độ cho địa chỉ này.': 'Không tìm thấy tọa độ cho địa chỉ này.',
  'Đã lấy tọa độ': 'Đã lấy tọa độ',

  'Không tải được dữ liệu. Kiểm tra backend endpoint.': 'Không tải được dữ liệu. Kiểm tra backend endpoint.',
  'Không lưu được dữ liệu. Kiểm tra dữ liệu nhập hoặc API.': 'Không lưu được dữ liệu. Kiểm tra dữ liệu nhập hoặc API.',
  'Không deactivate được dữ liệu.': 'Không ẩn được dữ liệu.',
  'Đang tải dữ liệu...': 'Đang tải dữ liệu...',
  'Chưa có dữ liệu': 'Chưa có dữ liệu',

  'Are you sure you want to edit this record?': 'Bạn có chắc muốn sửa dữ liệu này không?',
'Are you sure you want to hide this record?': 'Bạn có chắc muốn ẩn dữ liệu này không?',
'Are you sure you want to save these changes?': 'Bạn có chắc muốn lưu các thay đổi này không?',
'Are you sure you want to restore this record?': 'Bạn có chắc muốn mở lại dữ liệu này không?',
'Không restore được dữ liệu.': 'Không mở lại được dữ liệu.',


  'Bản đồ': 'Bản đồ',
  QR: 'QR',
  'Cài đặt': 'Cài đặt',
  Email: 'Email',
  Password: 'Mật khẩu',

  'Tổng quan dữ liệu hệ thống.': 'Tổng quan dữ liệu hệ thống.',
  'CRUD bảng danh mục lookup để không hard-code constant trong database.': 'CRUD bảng danh mục lookup để không hard-code constant trong database.',
  'Quản lý địa điểm, tọa độ, bán kính, priority, debounce và cooldown.': 'Quản lý địa điểm, tọa độ, bán kính, priority, debounce và cooldown.',
  'Quản lý nội dung thuyết minh gốc cho Place / Dish / General.': 'Quản lý nội dung thuyết minh gốc cho Place / Dish / General.',
  'Bản dịch theo từng ngôn ngữ. Nếu chưa có audio file, FE sẽ dùng TTS đọc translatedText.': 'Bản dịch theo từng ngôn ngữ. Nếu chưa có audio file, FE sẽ dùng TTS đọc translatedText.',
  'AudioUrl là tùy chọn. Nếu không có audio, mobile app dùng Web Speech API để đọc text.': 'Audio URL là tùy chọn. Nếu không có audio, mobile app dùng Web Speech API để đọc text.',
  'QR có thể trỏ tới Place, Dish hoặc Narration.': 'QR có thể trỏ tới Place, Dish hoặc Narration.',
  'Duyệt hoặc từ chối feedback của khách anonymous.': 'Duyệt hoặc từ chối feedback của khách anonymous.',
  'Log thời gian thực khi khách vào/gần/rời POI.': 'Log thời gian thực khi khách vào/gần/rời POI.',
  'Theo dõi lượt nghe theo QR / Geofence / Manual.': 'Theo dõi lượt nghe theo QR / Geofence / Manual.',

  'Created successfully.': 'Tạo mới thành công.',
'Updated successfully.': 'Cập nhật thành công.',
'Hidden successfully.': 'Ẩn dữ liệu thành công.',
'Restored successfully.': 'Mở lại dữ liệu thành công.',
'Validation failed.': 'Dữ liệu chưa hợp lệ.',

'Title is required.': 'Vui lòng nhập tiêu đề.',
'Original Text is required.': 'Vui lòng nhập nội dung gốc.',
'Content Type is required.': 'Vui lòng chọn loại nội dung.',
'Place is required for Place Narration.': 'Vui lòng chọn địa điểm cho thuyết minh địa điểm.',
'Dish is required for Dish Narration.': 'Vui lòng chọn món ăn cho thuyết minh món ăn.',
'Place must be empty for Dish Narration.': 'Thuyết minh món ăn không được chọn địa điểm.',
'Dish must be empty for Place Narration.': 'Thuyết minh địa điểm không được chọn món ăn.',
'Place must be empty for General Narration.': 'Thuyết minh chung không được chọn địa điểm.',
'Dish must be empty for General Narration.': 'Thuyết minh chung không được chọn món ăn.',

'QR Code Value is required.': 'Vui lòng nhập giá trị mã QR.',
'Target Type is required.': 'Vui lòng chọn loại đối tượng.',
'Place is required for Place QR.': 'Vui lòng chọn địa điểm cho QR địa điểm.',
'Dish is required for Dish QR.': 'Vui lòng chọn món ăn cho QR món ăn.',
'Narration is required for Narration QR.': 'Vui lòng chọn thuyết minh cho QR thuyết minh.',
'Dish must be empty for Place QR.': 'QR địa điểm không được chọn món ăn.',
'Narration must be empty for Place QR.': 'QR địa điểm không được chọn thuyết minh.',
'Place must be empty for Dish QR.': 'QR món ăn không được chọn địa điểm.',
'Narration must be empty for Dish QR.': 'QR món ăn không được chọn thuyết minh.',
'Place must be empty for Narration QR.': 'QR thuyết minh không được chọn địa điểm.',
'Dish must be empty for Narration QR.': 'QR thuyết minh không được chọn món ăn.',

'Translated Title is required.': 'Vui lòng nhập tiêu đề bản dịch.',
'Translated Text is required.': 'Vui lòng nhập nội dung bản dịch.',
'Translation Source is required.': 'Vui lòng chọn nguồn dịch.',
'Translation is required.': 'Vui lòng chọn bản dịch.',
'Audio URL is required.': 'Vui lòng nhập URL âm thanh.',

'Place Name is required.': 'Vui lòng nhập tên địa điểm.',
'Place Type is required.': 'Vui lòng chọn loại địa điểm.',
'Trigger Mode is required.': 'Vui lòng chọn chế độ kích hoạt.',
'Latitude is required for POI/geofence places.': 'Vui lòng nhập vĩ độ cho địa điểm POI/geofence.',
'Longitude is required for POI/geofence places.': 'Vui lòng nhập kinh độ cho địa điểm POI/geofence.',
'Radius Meters must be greater than 0.': 'Bán kính phải lớn hơn 0.',
'Priority must be greater than or equal to 0.': 'Độ ưu tiên phải lớn hơn hoặc bằng 0.',
'Debounce Seconds must be greater than or equal to 0.': 'Debounce phải lớn hơn hoặc bằng 0.',
'Cooldown Seconds must be greater than or equal to 0.': 'Cooldown phải lớn hơn hoặc bằng 0.',

'Category Name is required.': 'Vui lòng nhập tên danh mục.',
'Dish Name is required.': 'Vui lòng nhập tên món ăn.',
'Category is required.': 'Vui lòng chọn danh mục.',
'Average Price must be greater than or equal to 0.': 'Giá trung bình phải lớn hơn hoặc bằng 0.',
'Price must be greater than or equal to 0.': 'Giá phải lớn hơn hoặc bằng 0.',
'Download QR': 'Tải QR',
'QR Code Value is empty.': 'Giá trị mã QR đang trống.',
'Unable to generate QR image.': 'Không thể tạo ảnh QR.',
'Showing': 'Hiển thị',
'Previous': 'Trước',
'Next': 'Sau',
'Page': 'Trang',
};

const enText: Record<string, string> = {
  Admin: 'Admin',
  'Quản trị': 'Admin',
  Dashboard: 'Dashboard',
  'Bảng điều khiển': 'Dashboard',
  Lookup: 'Lookup',
  'Danh mục': 'Lookup',
  Languages: 'Languages',
  'Ngôn ngữ': 'Languages',
  'Places / POI': 'Places / POI',
  'Địa điểm / POI': 'Places / POI',
  Dishes: 'Dishes',
  'Món ăn': 'Dishes',
  Narrations: 'Narrations',
  'Thuyết minh': 'Narrations',
  Translations: 'Translations',
  'Bản dịch': 'Translations',
  Audio: 'Audio',
  'Âm thanh': 'Audio',
  'QR Codes': 'QR Codes',
  'Mã QR': 'QR Codes',
  Feedbacks: 'Feedbacks',
  Feedback: 'Feedback',
  Listening: 'Listening',
  'Lượt nghe': 'Listening',
  Geofence: 'Geofence',

  Language: 'Language',
  Vietnamese: 'Vietnamese',
  English: 'English',
  'Tiếng Việt': 'Vietnamese',

  'Admin Dashboard': 'Admin Dashboard',
  'Bảng điều khiển quản trị': 'Admin Dashboard',
  'Language Management': 'Language Management',
  'Quản lý ngôn ngữ': 'Language Management',
  'The narration languages are fixed for the system. Admin cannot create new languages on this screen.': 'The narration languages are fixed for the system. Admin cannot create new languages on this screen.',
'Danh sách ngôn ngữ thuyết minh được cố định cho hệ thống. Admin không tạo mới ngôn ngữ tại màn hình này.': 'The narration languages are fixed for the system. Admin cannot create new languages on this screen.',

'Supported system languages': 'Supported system languages',
'Ngôn ngữ hệ thống hỗ trợ': 'Supported system languages',

'These languages are used for narration translations and guest language selection.': 'These languages are used for narration translations and guest language selection.',
'Các ngôn ngữ này sẽ được dùng cho bản dịch thuyết minh và lựa chọn ngôn ngữ nghe của khách.': 'These languages are used for narration translations and guest language selection.',

'Display Name': 'Display Name',
'Tên hiển thị': 'Display Name',

'System Label': 'System Label',
'Nhãn hệ thống': 'System Label',

'TTS lang': 'TTS lang',

'DB Status': 'DB Status',
'Trạng thái DB': 'DB Status',

'Active in DB': 'Active in DB',
'Đang hoạt động': 'Active in DB',

'Exists in DB but disabled': 'Exists in DB but disabled',
'Có trong DB nhưng đang tắt': 'Exists in DB but disabled',

'Missing in DB': 'Missing in DB',
'Chưa có trong DB': 'Missing in DB',

'Languages outside fixed configuration': 'Languages outside fixed configuration',
'Ngôn ngữ không thuộc cấu hình cố định': 'Languages outside fixed configuration',

'These rows should be disabled by seed_supported_languages_fixed.sql. They should not be used for the public app.': 'These rows should be disabled by seed_supported_languages_fixed.sql. They should not be used for the public app.',
'Các dòng này nên được tắt bằng file seed_supported_languages_fixed.sql. Không nên dùng cho app public.': 'These rows should be disabled by seed_supported_languages_fixed.sql. They should not be used for the public app.',
  'Lookup Management': 'Lookup Management',
  'Quản lý danh mục': 'Lookup Management',
  'Places / POI Management': 'Places / POI Management',
  'Quản lý địa điểm / POI': 'Places / POI Management',
  'Dish Categories': 'Dish Categories',
  'Danh mục món ăn': 'Dish Categories',
  'Place Dishes': 'Place Dishes',
  'Món ăn theo địa điểm': 'Place Dishes',
  'Narration Management': 'Narration Management',
  'Quản lý thuyết minh': 'Narration Management',
  'Translation Management': 'Translation Management',
  'Quản lý bản dịch': 'Translation Management',
  'Audio Management': 'Audio Management',
  'Quản lý âm thanh': 'Audio Management',
  'QR Code Management': 'QR Code Management',
  'Quản lý mã QR': 'QR Code Management',
  'Feedback Management': 'Feedback Management',
  'Quản lý feedback': 'Feedback Management',
  'Geofence Events': 'Geofence Events',
  'Sự kiện geofence': 'Geofence Events',
  'Listening Histories': 'Listening Histories',
  'Lịch sử nghe': 'Listening Histories',

'Are you sure you want to edit this record?': 'Are you sure you want to edit this record?',
'Are you sure you want to hide this record?': 'Are you sure you want to hide this record?',
'Are you sure you want to save these changes?': 'Are you sure you want to save these changes?',
'Are you sure you want to restore this record?': 'Are you sure you want to restore this record?',
'Không restore được dữ liệu.': 'Unable to restore this record.',

  'Place Types': 'Place Types',
  'Loại địa điểm': 'Place Types',
  'Content Types': 'Content Types',
  'Loại nội dung': 'Content Types',
  'Target Types': 'Target Types',
  'Loại đối tượng': 'Target Types',
  'Translation Sources': 'Translation Sources',
  'Nguồn dịch': 'Translation Sources',
  'Trigger Modes': 'Trigger Modes',
  'Chế độ kích hoạt': 'Trigger Modes',
  'Geofence Event Types': 'Geofence Event Types',
  'Loại sự kiện geofence': 'Geofence Event Types',
  'Geofence Event Statuses': 'Geofence Event Statuses',
  'Trạng thái sự kiện geofence': 'Geofence Event Statuses',

  'Thêm dữ liệu': 'Add data',
  'Cập nhật dữ liệu': 'Update data',
  'Tạo mới': 'Create',
  'Lưu thay đổi': 'Save changes',
  'Hủy': 'Cancel',
  'Sửa': 'Edit',
  'Ẩn': 'Hide',
  'Mở': 'Restore',
  'Thao tác': 'Actions',

  'Add data': 'Add data',
  'Update data': 'Update data',
  Create: 'Create',
  'Save changes': 'Save changes',
  Cancel: 'Cancel',
  Edit: 'Edit',
  Hide: 'Hide',
  Restore: 'Restore',
  Actions: 'Actions',
  Approve: 'Approve',
  Reject: 'Reject',
  'Update status': 'Update status',

  Yes: 'Yes',
  No: 'No',
  On: 'On',
  Off: 'Off',
  Có: 'Yes',
  Không: 'No',
  Bật: 'On',
  Tắt: 'Off',

  Id: 'Id',
  ID: 'Id',
  Code: 'Code',
  Mã: 'Code',
  Name: 'Name',
  Tên: 'Name',
  Default: 'Default',
  'Mặc định': 'Default',
  Active: 'Active',
  'Hoạt động': 'Active',
  Description: 'Description',
  'Mô tả': 'Description',
  Address: 'Address',
  'Địa chỉ': 'Address',
  Latitude: 'Latitude',
  'Vĩ độ': 'Latitude',
  Longitude: 'Longitude',
  'Kinh độ': 'Longitude',
  Lat: 'Latitude',
  Lng: 'Longitude',
  Price: 'Price',
  Giá: 'Price',
  Category: 'Category',
  Recommended: 'Recommended',
  'Gợi ý': 'Recommended',
  Status: 'Status',
  'Trạng thái': 'Status',
  Rating: 'Rating',
  'Đánh giá': 'Rating',
  Comment: 'Comment',
  'Bình luận': 'Comment',
  Note: 'Note',
  'Ghi chú': 'Note',
  Type: 'Type',
  Source: 'Source',
  Target: 'Target',
  Guest: 'Guest',
  Distance: 'Distance',
  Detected: 'Detected',
  Duration: 'Duration',
  Time: 'Time',
  URL: 'URL',
  Format: 'Format',
  Value: 'Value',
  Approved: 'Approved',

  'Language Code': 'Language Code',
  'Mã ngôn ngữ': 'Language Code',
  'Language Name': 'Language Name',
  'Tên ngôn ngữ': 'Language Name',
  'Language Id': 'Language Id',
  'Id ngôn ngữ': 'Language Id',

  'Place Id': 'Place Id',
  'Id địa điểm': 'Place Id',
  'Place Name': 'Place Name',
  'Tên địa điểm': 'Place Name',
  Place: 'Place',
  'Địa điểm': 'Place',
  'Place Type Id': 'Place Type Id',
  'Id loại địa điểm': 'Place Type Id',
  'Opening Hours': 'Opening Hours',
  'Giờ mở cửa': 'Opening Hours',
  'Image URL': 'Image URL',
  'URL hình ảnh': 'Image URL',
  'Is POI': 'Is POI',
  'Là POI': 'Is POI',
  'Geofence Enabled': 'Geofence Enabled',
  'Bật geofence': 'Geofence Enabled',
  'Radius Meters': 'Radius Meters',
  'Bán kính mét': 'Radius Meters',
  Priority: 'Priority',
  'Độ ưu tiên': 'Priority',
  'Trigger Mode Id': 'Trigger Mode Id',
  'Id chế độ kích hoạt': 'Trigger Mode Id',
  'Debounce Seconds': 'Debounce Seconds',
  'Thời gian debounce': 'Debounce Seconds',
  'Cooldown Seconds': 'Cooldown Seconds',
  'Thời gian cooldown': 'Cooldown Seconds',

  'Dish Id': 'Dish Id',
  'Id món ăn': 'Dish Id',
  'Dish Name': 'Dish Name',
  'Tên món ăn': 'Dish Name',
  Dish: 'Dish',
  'Category Id': 'Category Id',
  'Id danh mục': 'Category Id',
  'Category Name': 'Category Name',
  'Tên danh mục': 'Category Name',
  'Average Price': 'Average Price',
  'Giá trung bình': 'Average Price',
  'Signature Dish': 'Signature Dish',
  'Món đặc trưng': 'Signature Dish',
  Signature: 'Signature',

  'Narration Id': 'Narration Id',
  'Id thuyết minh': 'Narration Id',
  Narration: 'Narration',
  'Original Text': 'Original Text',
  'Nội dung gốc': 'Original Text',
  'Content Type Id': 'Content Type Id',
  'Id loại nội dung': 'Content Type Id',
  'Created By Admin Id': 'Created By Admin Id',
  'Id admin tạo': 'Created By Admin Id',

  'Translation Id': 'Translation Id',
  'Id bản dịch': 'Translation Id',
  Translation: 'Translation',
  'Translated Title': 'Translated Title',
  'Tiêu đề đã dịch': 'Translated Title',
  'Translated Text': 'Translated Text',
  'Nội dung đã dịch': 'Translated Text',
  'Translation Source Id': 'Translation Source Id',
  'Id nguồn dịch': 'Translation Source Id',
  'Reviewed By': 'Reviewed By',
  'Người duyệt': 'Reviewed By',
  Reviewed: 'Reviewed',
  'Đã duyệt': 'Reviewed',

  'Audio URL': 'Audio URL',
  'URL âm thanh': 'Audio URL',
  AudioUrl: 'Audio URL',
  'Voice Name': 'Voice Name',
  'Tên giọng đọc': 'Voice Name',
  'Voice Gender': 'Voice Gender',
  'Giới tính giọng đọc': 'Voice Gender',
  'Duration Seconds': 'Duration Seconds',
  'Thời lượng giây': 'Duration Seconds',
  'File Format': 'File Format',
  'Định dạng file': 'File Format',
  'Generated By': 'Generated By',
  'Tạo bởi': 'Generated By',

  'QR Code Value': 'QR Code Value',
  'Giá trị mã QR': 'QR Code Value',
  'QR Image URL': 'QR Image URL',
  'URL hình QR': 'QR Image URL',
  'Target Type Id': 'Target Type Id',
  'Target Type': 'Target Type',

  'Tự động lấy từ địa chỉ': 'Automatically resolved from address',
  'Auto-fill from address or enter manually': 'Auto-fill from address or enter manually',
'Tự động lấy từ địa chỉ hoặc nhập tay': 'Auto-fill from address or enter manually',
  'Lấy tọa độ từ địa chỉ': 'Resolve coordinates from address',
  'Vui lòng nhập địa chỉ trước.': 'Please enter the address first.',
  'Không tìm thấy tọa độ cho địa chỉ này.': 'No coordinates were found for this address.',
  'Đã lấy tọa độ': 'Coordinates resolved',

  'Không tải được dữ liệu. Kiểm tra backend endpoint.': 'Unable to load data. Please check the backend endpoint.',
  'Không lưu được dữ liệu. Kiểm tra dữ liệu nhập hoặc API.': 'Unable to save data. Please check the input or API.',
  'Không deactivate được dữ liệu.': 'Unable to deactivate this record.',
  'Đang tải dữ liệu...': 'Loading data...',
  'Chưa có dữ liệu': 'No data available',

  'Bản đồ': 'Map',
  QR: 'QR',
  'Cài đặt': 'Settings',
  Email: 'Email',
  Password: 'Password',

  'Tổng quan dữ liệu hệ thống.': 'System data overview.',
  'CRUD bảng danh mục lookup để không hard-code constant trong database.': 'CRUD lookup tables so constants are not hard-coded in the database.',
  'Quản lý địa điểm, tọa độ, bán kính, priority, debounce và cooldown.': 'Manage places, coordinates, radius, priority, debounce, and cooldown.',
  'Quản lý nội dung thuyết minh gốc cho Place / Dish / General.': 'Manage original narration content for Place / Dish / General.',
  'Bản dịch theo từng ngôn ngữ. Nếu chưa có audio file, FE sẽ dùng TTS đọc translatedText.': 'Manage translations by language. If no audio file exists, the frontend will use TTS to read translated text.',
  'AudioUrl là tùy chọn. Nếu không có audio, mobile app dùng Web Speech API để đọc text.': 'Audio URL is optional. If no audio file exists, the mobile app uses the Web Speech API to read text.',
  'QR có thể trỏ tới Place, Dish hoặc Narration.': 'QR codes can point to a place, dish, or narration.',
  'Duyệt hoặc từ chối feedback của khách anonymous.': 'Approve or reject anonymous guest feedback.',
  'Log thời gian thực khi khách vào/gần/rời POI.': 'Real-time logs when guests enter, approach, or leave a POI.',
  'Theo dõi lượt nghe theo QR / Geofence / Manual.': 'Track listening sessions by QR, geofence, or manual playback.',

  'Created successfully.': 'Created successfully.',
'Updated successfully.': 'Updated successfully.',
'Hidden successfully.': 'Hidden successfully.',
'Restored successfully.': 'Restored successfully.',
'Validation failed.': 'Validation failed.',
'Download QR': 'Download QR',
'QR Code Value is empty.': 'QR Code Value is empty.',
'Unable to generate QR image.': 'Unable to generate QR image.',
'Showing': 'Showing',
'Previous': 'Previous',
'Next': 'Next',
'Page': 'Page',
};

export function normalizeUiLanguage(value?: string | null): UiLanguage {
  const code = value?.toLowerCase() ?? '';

  if (code.startsWith('vi')) return 'vi';
  if (code.startsWith('ja')) return 'ja';
  if (code.startsWith('ko')) return 'ko';
  if (code.startsWith('zh') || code.startsWith('cn')) return 'zh';

  return 'en';
}

export function translateKey(key: string, language: UiLanguage): string {
  return keyTranslations[language][key] ?? key;
}

export function translateText(text: string, language: UiLanguage): string {
  // Admin text chỉ hỗ trợ VI / EN.
  // Ưu tiên viText/enText, sau đó fallback sang keyTranslations
  // để dùng lại các key đã khai báo sẵn như "Chọn dữ liệu", "Không chọn Place"...
  const activeLanguage: Extract<UiLanguage, 'vi' | 'en'> = language === 'vi' ? 'vi' : 'en';
  const dictionary = activeLanguage === 'vi' ? viText : enText;

  return dictionary[text] ?? keyTranslations[activeLanguage]?.[text] ?? text;
}
